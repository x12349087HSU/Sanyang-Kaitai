...
跟使用者討論過的下一步：
1. **（已試過，2026-09-02，無效）先試最低成本的方案**：去 FinMind 官網
   免費註冊帳號拿一組 token，填進 Render 的環境變數（`config.py` 已經
   支援 `FINMIND_TOKEN`）。使用者實際申請並設定後重新測試「0050」，結果
   50 檔裡仍有 13 檔失敗，**全部 13 檔的錯誤訊息跟沒有 token 之前一模
   一樣**（FinMind 402 + 證交所備援 428），證實 FinMind 對 Render 這類
   雲端主機 IP 的封鎖是**整批性質，不分有沒有註冊 token**，不是「匿名 vs.
   已註冊」的差別待遇問題。這條路已經排除，不用再嘗試。
2. **後續討論過的完整選項清單（尚未決定，還在考慮）**：
   - **Tailscale 連回家裡電腦**：讓 API 從住宅 ISP IP 發出請求（目前看
     起來沒被擋），免費、改動小（裝 Tailscale + 改 App 裡的 API 網址）。
     取捨：使用者用 App 的**當下**這台電腦必須是醒著、API process 要
     一直在跑，單點依賴重；而且完全沒解決「證交所依請求量觸發流量管制」
     這件事，只是換一個 IP 去撞同一道牆。
   - **換一個雲端主機賭賭看**（Fly.io/Railway/DigitalOcean 等）：很多
     防爬蟲機制是用「這個 IP 是不是雲端/機房」的通用資料庫判斷，不是
     針對 Render 特別擋，換平台大概率還是會遇到同樣問題，屬於賭運氣。
   - **付費的住宅代理伺服器服務**：API 還留在 Render，但打 FinMind/
     證交所的請求改成透過付費代理轉一手，讓對方看到住宅 IP。優點是
     電腦不用開機；缺點是要額外付費（流量計費，實際金額沒測過）。
   - **付費升級 FinMind 方案**：沒驗證過付費帳號的請求待遇是否跟免費
     token 不同，風險是要先付錢才能驗證。
   - **（目前傾向的方向）改成「家裡電腦每天排程抓一次資料、結果推上
     雲端，Render 全天候只負責供應現成結果」**：因為目前目標是收盤價
     （使用者已確認，不是即時報價），資料本來就一天只變一次，不需要
     每次都即時重新抓。做法是本機排程（例如收盤後）呼叫既有的
     `screen_0050()`/`screen_top150()`，把結果推到 GitHub，Render 的
     API 改成讀 GitHub 上最新的結果檔案回傳，而不是即時算。這個方案
     **額外的好處**是連帶降低觸發證交所流量管制的機率（一天只有一個
     來源抓一次，不會有大量即時請求），而且家裡電腦只需要在排程時間點
     開機跑幾分鐘，不是像 Tailscale 方案那樣要整天保持在跑；就算某天
     忘記開機，也只是資料舊一天，App 本身仍然打得到 Render，不會整個
     不能用，容錯空間比 Tailscale 方案大。代價是要多寫排程腳本、改 API
     讀資料的方式，工程量比 Tailscale 方案大。
   - 使用者還在權衡，**尚未做出最終決定**。
