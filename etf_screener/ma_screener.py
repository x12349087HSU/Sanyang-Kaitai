...
TIER_LABELS: dict[int, str] = {
    ...
    -3: "三聲無奈，請別凹單",
    -4: "四面楚歌，岌岌可危",
    # 0 不在 TIER_ORDER 裡（批次篩選 0050/top150 時刻意排除，只顯示八個
    # 有明確訊號的分類），但「直接篩選個股」查詢單一檔時，即使剛好落在
    # 這個中性分類，使用者也需要看到查詢結果，不能顯示成「查無資料」，
    # 見 screen_page.py render_screen_html() 的 include_neutral_tier 參數。
    0: "多空訊號不一致",
}
TIER_DESCRIPTIONS: dict[int, str] = {
    ...
    0: "最近一日收盤價站上/跌破的均線數量不構成上述八個等級的任何一種"
    "（例如站上 5MA 但跌破 10MA）",
}
